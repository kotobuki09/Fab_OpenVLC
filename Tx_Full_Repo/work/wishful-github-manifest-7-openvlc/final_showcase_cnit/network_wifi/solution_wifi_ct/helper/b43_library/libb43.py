...
class B43:
	...
	GOOD_FCS_MATCH_RA_COUNTER	= 0x0112		# RX Data "Frames" with Good FCS and Matching RA //non sono solo data frame ma tutti i frame
	...
	B43_MMIO_REV3PLUS_TSF_LOW = 0x180   #core rev >= 3 only
	B43_MMIO_REV3PLUS_TSF_HIGH = 0x184   #core rev >= 3 only

	B43_MMIO_IFSSLOT = 0x684	#Interframe slot time
	B43_MMIO_IFSCTL	= 0x688	#Interframe space control
	B43_MMIO_IFSSTAT = 0x690
	B43_MMIO_IFSMEDBUSYCTL = 0x692
	B43_MMIO_IFTXDUR = 0x694
	...
	def __init__(self, phy=None):
		...
		self.param_list = [self.TDMA_SUPER_FRAME_SIZE, self.TDMA_NUMBER_OF_SYNC_SLOT, self.TDMA_ALLOCATED_SLOT, self.CSMA_CW, self.CSMA_CW_MIN, self.CSMA_CW_MAX, self.MAC_ADDR_SYNCHRONIZATION_AP]
		return
	...
	def shmRead32Char(self, routing, offset):
		"""Do a 32bit SHM read"""
		try:
			self.f_shm32read.seek(0)
			self.f_shm32read.write("0x%X 0x%X" % (routing, offset))
