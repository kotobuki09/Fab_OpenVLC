...
def update_data_lte(attrname, old, new):
    ...
    plt_array[3].y_range.end = slider_y_range_lte.value


def update_data_tx_range(attrname, old, new):
    global plt_array
    #print(slider_y_range_lte.value)
    plt_array[2].y_range.end = slider_y_range_tx.value
...
fixed_colors = ["Blue", "Red", "Black", "Orange", "Purple", "Green", "Pink"]
...
nsu_cols = [
    ...
    TableColumn(field="load", title="Application Load", width=110),
]
...
slider_y_range_tx = Slider(value=2000.0, start=100.0, end=2000.0, step=100.0, title="Y range")
for w in [slider_y_range_tx]:
    w.on_change('value', update_data_tx_range)

tab_graph_conf = Panel(child=Row(button_group_graph_conf, Column(slider_y_range, slider_y_range_lte, slider_y_range_tx)), title="Conf")
...
button_group_wifi = RadioButtonGroup(labels=["OFF", "1", "2", "3", "4"], active=0, width=400)
...
tab_wifi_rate_button = Panel(child=button_group_wifi_rate, title="WiFi Rate")
...
button_group_deterministic = RadioButtonGroup(labels=["OFF", "ON"], active=0)
...
l1 = layout([[ tabs_wifi, div_blank_3, tabs_functionality]], sizing_mode='fixed', width=100)
...
plots = [[
    [tabs_channel_trace],
    [div_blank_4,                   button_group_channel_trace, div_blank_2],
    [tabs_active_networks_table,    l1] ]]
...
#plot_width_value = 430

plot_height_value = 280
plot_width_value = 800


for net in conf.networks:
    print("create plot", net)

    thr_plt = figure(
        plot_height=plot_height_value, plot_width=plot_width_value,
        title="{} Received Throughput".format(net),
    ...
    tx_frame_plt = figure(
        plot_height=plot_height_value, plot_width=plot_width_value,
        title="{} Number of TX frame".format(net),
        # tools="crosshair,pan,reset,save,wheel_zoom",
        tools="",
        toolbar_location=None,
        x_axis_type="datetime",
    ...
    tx_frame_plt.xaxis.axis_label = "Time"
    tx_frame_plt.yaxis.axis_label = "NUM TX FRAME"

    for k in conf.networks[net]:
        ...
        thr_plt.line(
            'timestamp', 'THR',
            source=data,
            legend=conf.networks[net][k]['hrn'],
    ...
    tx_frame_plt.x_range.follow = "end"
    tx_frame_plt.x_range.follow_interval = 60000
    tx_frame_plt.legend.location = "top_left"

    plt_array.append(thr_plt)
    plt_array.append(per_plt)
    plt_array.append(tx_frame_plt)


for stat in conf.cumulative:
    print("create plot", stat)
    cumulative_thr_plt = figure(
        plot_height=plot_height_value, plot_width=plot_width_value,
        title="{} Throughput".format(stat),
        # tools="crosshair,pan,reset,save,wheel_zoom",
        tools="",
        toolbar_location=None,
        x_axis_type="datetime",
    ...
    cumulative_thr_plt.xaxis.axis_label = "Time"
    cumulative_thr_plt.yaxis.axis_label = "Throughput [B/s]"
    cumulative_thr_plt.yaxis[0].formatter = NumeralTickFormatter(format='0.0b')

    for k in conf.cumulative[stat]:
        data = ColumnDataSource(
            data=dict(
                timestamp=[],
                THR=[],
        ...
        cumulative_thr_plt.line(
            'timestamp', 'THR',
            source=data,
            legend=conf.cumulative[stat][k]['hrn'],
            line_color=fixed_colors[index_color],
            line_width=3, line_alpha=0.6,
        )
        index_color += 1

    cumulative_thr_plt.x_range.follow = "end"
    cumulative_thr_plt.x_range.follow_interval = 60000
    cumulative_thr_plt.legend.location = "top_left"

    plt_array.append(cumulative_thr_plt)

#plots.append([plt_array[3], plt_array[0], plt_array[1], plt_array[2]])
plots.append([plt_array[3], plt_array[1]])
plots.append([plt_array[0], plt_array[2]])
