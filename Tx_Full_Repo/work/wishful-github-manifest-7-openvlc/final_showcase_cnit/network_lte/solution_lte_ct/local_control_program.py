...
def remote_control_program(controller):
        ...
        while getattr(reading_thread, "do_run", True):
            ...
            try:
                ...
                if ready[0]:
                    ...
                    if "PDSCH-BLER" in stats:
                        if stats["PDSCH-BLER"] <= 5:
                            reading_buffer[0] = (1 - (stats["PDSCH-BLER"] / 5))
                        else:
                            reading_buffer[0] = 1
    ...
    def rcv_from_iperf_socket(iperf_througputh):
        ...
        #iperf_server_ip_address = "10.8.9.14"
        context = zmq.Context()
        ...
        while getattr(iperf_thread, "do_run", True):
            if poller.poll(2000):  # 1s timeout in milliseconds
    ...
    while not controller.is_stopped():
        ...
        if True:
            report_message = {"measure": [time.time(), iperf_througputh[0], reading_buffer[0] ],  "mac_address": (my_mac)}
            # print(report_message)
            controller.send_upstream(report_message)
