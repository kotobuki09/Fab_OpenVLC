...
#controller.send_upstream({ "measure" : [[0, station_w[0], station_claim[0], station_offer[0], report_stats['thr'], report_stats['cw'], report_stats['busy_time'], report_stats['a_coll'], report_stats['e_ave']]], "mac_address" : (my_mac) })
#define data field
	...
	for meas_type_id in range(number_type_measurements):
		...
		if measurement_types[meas_type_id] == "CW" :
			...
			if nodeIp=="192.168.0.1" or nodeIp=="192.168.0.3" or nodeIp=="192.168.0.5":
				...
				plt.tight_layout()
				#legend = ax.legend(loc='lower center', bbox_to_anchor=(0.5, 1), ncol=5, fancybox=True, shadow=True)

		#extract throughput information
		if measurement_types[meas_type_id] == "a_coll" :
			num_thr_yaxis = []
			if nodeIp=="192.168.0.1" or nodeIp=="192.168.0.3" or nodeIp=="192.168.0.5":
				print(" node : " + nodeIp + " ( plot --> " + measurement_types[meas_type_id] + " )")
...
plt.tight_layout()
#plt.show()
fig_filename="fig_%s.pdf" % 'react-experiment-result'
