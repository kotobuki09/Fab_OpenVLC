...
"""
import types
# from runtime.runtime import LocalManager
# from master.master import GlobalManager
# from upis.upi_rn import UPI_RN
# from upis.upi_rn import radio_info_t
# from upis.upi_rn import radio_platform_t
# from upis.upi_rn import execution_engine_t
# from upis.upi_rn import radio_program_t
...
def getPlatformInformation(node, log, controller):
    ...
    log.debug('***************** %s ***************' % getPlatformInformation.__name__)
    ...
    current_radio_list_string = controller.nodes(node).radio.iface("wlan0").get_radio_platforms()
    ...
    current_platform_info_str = controller.nodes(node).radio.iface("wlan0").get_radio_info(current_radio_list[wmp_platform_index].platform_id)
...
def active_CSMA_radio_program(node, log, global_mgr, current_platform_info):
    ...
    # all functions are execute immediately
    exec_time = None
    ...
    #     rvalue = rvalue[str(node)]
    #     if rvalue == SUCCESS :
    #         log.debug('Radio program activation successful')
    #     else :
    #         log.debug('Error in radio program activation')
    #         return False
    # except Exception as e:
    #     log.fatal("An error occurred in radio program activation: %s" % e)
    #     return False

    return True


def active_CSMA_radio_program_slot_1(node, log, global_mgr):
    """ Active CSMA radio program to the node passed by argument parameter.
        To enable a radio program on WMP platform we need two different action, inject the radio program and activate it.
...
def active_TDMA_radio_program_slot_2(node, log, global_mgr):
...
def active_TDMA_radio_program(node, log, global_mgr, current_platform_info):
    """ Active TDMA radio program to the node passed by argument parameter.
        To enable a radio program on WMP platform we need two different action, inject the radio program and activate it.
    ...
    for ii in range(len(current_platform_info.radio_program_list)):
        if current_platform_info.radio_program_list[ii].radio_prg_name == "TDMA" :
            log.debug("%s", current_platform_info.radio_program_list[ii].radio_prg_pointer )
...
def set_TDMA_parameters(node, log, global_mgr, tdma_params):
    ...
    # UPIargs = { 'interface' : 'wlan0', UPI_RN.TDMA_SUPER_FRAME_SIZE: tdma_params['TDMA_SUPER_FRAME_SIZE'], UPI_RN.TDMA_NUMBER_OF_SYNC_SLOT : tdma_params['TDMA_NUMBER_OF_SYNC_SLOT'], UPI_RN.TDMA_ALLOCATED_SLOT : tdma_params['TDMA_ALLOCATED_SLOT'] }
    # try:
    #     rvalue = global_mgr.runAt(node, UPIfunc, UPIargs, exec_time)
    #     log.warning('The change parameter is %s' % str(rvalue))
    # except Exception as e:
    #     log.fatal("An error occurred in TDMA parameters update : %s" % e)
    #     return False
