...
if [ 1 -eq 1 ]; then
    ...
    for sta in $stations
    do
        rsync -avz  --exclude final_showcase/ --exclude final_showcase_cnit/ --exclude dev/ --exclude=.git --exclude '*.o' --exclude '*.h' --exclude '*.c' --exclude '*.pyc' --exclude examples/* --exclude agent_modules/*  --exclude .repo/ ../../../../  -e ssh root@$sta:~/wishful-github-manifest/
