...
def collect_remote_messages(lcpDescriptor_node, socket_visualizer):
    ...
    while getattr(receive_thread, "do_run", True):
        ...
        if msg:
            ...
            if msg['measure'][0][2] != 0.0 and str(msg['wlan_ip_address']) != '192.168.3.107':
        ...
        if len(nodes) == num_testbed_nodes:
            ...
            while do_run:
                ...
                if SYSTEM_STATE[0] == 'MONITOR_NETWORK_ACTIVATION':
                    ...
                    #blocking(False)
                    controller.delay(2).nodes(mytestbed.wmp_nodes1[0]).net.start_packetflow('192.168.4.' + mytestbed.ap_node1.ip.split('.')[3],
                                                                    '1234', '3000', '100M', '1300')
                    for i in range(len(mytestbed.wmp_nodes)):
                        if WITH_BACK_TRAFFIC:
                            controller.delay(2).nodes(mytestbed.wmp_nodes[i]).net.start_packetflow('192.168.3.' + mytestbed.ap_node.ip.split('.')[3],
                                                                        '1234', '3000', '100M', '1300')

                        else:
                            if str(mytestbed.wmp_nodes[i].ip) == '192.168.2.10':
                                controller.delay(2).nodes(mytestbed.wmp_nodes[i]).net.start_packetflow('192.168.3.' + mytestbed.ap_node.ip.split('.')[3],
                                                                        '1234', '3000', '100M', '1300')
                ...
                if SYSTEM_STATE[0] == 'SEND_DEACTIVATION_MESSAGE':
                    ...
                    for ii in range(len(wmp_node_tot)):
                        if (str(node_deactivated[0]) == 'extensa' and wmp_node_tot[ii].ip == '192.168.2.10') or (str(node_deactivated[0]) == 'nautilus' and wmp_node_tot[ii].ip == '192.168.2.1'):
                            break
                        index += 1
                    log.info("Deactivate " + str(node_deactivated[0]))
