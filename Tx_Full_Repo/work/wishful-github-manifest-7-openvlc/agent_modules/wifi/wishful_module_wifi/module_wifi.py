...
class WifiModule(wishful_module.AgentModule):
    ...
    def start_hostapd(self, file_path):
        ...
        cmd_str = 'sudo ifconfig wlan0 down & sudo hostapd -B ' + str(file_path)
        try:
            [rcode, sout, serr] = self.run_command(cmd_str)
    ...
    def network_dump(self, iface):
        ...
        self.log.info('setting ip routing rules on %s' % (hostname))
        cmd_str1 = ''
        if hostname == 'alix17':
            cmd_str1 = 'ip route add 10.8.7.2 via 192.168.4.112 dev wlan0'
        elif hostname == 'alix15':
            cmd_str1 = 'ip route add 10.8.7.2 via 192.168.3.114 dev wlan0' 
        try:
            [rcode, sout, serr] = self.run_command(cmd_str)
            [rcode, sout, serr] = self.run_command(cmd_str1)
        except Exception as e:
            fname = inspect.currentframe().f_code.co_name
            self.log.fatal("An error occurred in %s: %s" % (fname, e))
            raise exceptions.UPIFunctionExecutionFailedException(func_name=fname, err_msg=str(e))
